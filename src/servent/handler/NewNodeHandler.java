		...
		try {
			...
			if (clientMessage.getMessageType() == MessageType.NEW_NODE) {
				...
				if (isMyPred) { //if yes, prepare and send welcome message
					...
					Map<Integer, Map<String, MetaFile>> myValues = AppConfig.chordState.getValueMap();
					Map<Integer, Map<String, MetaFile>> hisValues = new HashMap<>();

					int myId = AppConfig.myServentInfo.getChordId();
					int hisPredId = hisPred.getChordId();
					int newNodeId = newNodeInfo.getChordId();

					for (Entry<Integer, Map<String, MetaFile>> valueEntry : myValues.entrySet()) {
